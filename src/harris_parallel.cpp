...
)
{
  ...
  for (int index_img = 0; index_img < nbr_imgs; index_img++){
   ...
   float *Ixs = new float[nx*ny*nbr_imgs];
   float *Iys = new float[nx*ny*nbr_imgs];
   for (int i =0; i<nbr_imgs; i++)
   {
    for (int j=0; j<nx*ny;j++){
      Ixs[i*nx*ny +j] = Ix[i][j];
      Iys[i*nx*ny +j] = Iy[i][j];
    }
   }
  float * As = new float[nx*ny*nbr_imgs];
  float * Bs = new float[nx*ny*nbr_imgs];
  float * Cs = new float[nx*ny*nbr_imgs];

  message(" 3.a.Computing the autocorrelation with cuda: \t", start, end, verbose);
  compute_autocorrelation_matrix_cuda(Ixs, Iys, As, Bs, Cs, nx, ny, nbr_imgs);
      
  message(" 3.b. Computing gaussians with cuda: \t", start, end, verbose);
   // O(nx*ny*nbr_imgs*3)

  
  parallel_comp_cuda(As, Bs, Cs, nx, ny, nbr_imgs, sigma_i, gauss);

  message(" Allocation A, B, C: \t", start, end, verbose);
  for (int i =0; i<nbr_imgs; i++)
   {
    for (int j=0; j<nx*ny;j++){ 
        A[i][j] = As[i*nx*ny +j];
...
{
  printf("start harris scale\n");

    if(Nscales<=1 || nx<=64 || ny<=64)
    {

      printf("start harris parallel");  
      harris_parallel(
